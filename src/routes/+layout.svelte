...
<div class="relative z-0 m-auto h-full min-h-screen w-full justify-between">
	...
	<div class="relative mx-auto max-w-6xl">
		<nav class="z-30 flex h-16 w-full items-center justify-between bg-opacity-5 px-2">
			<!-- Logo -->
			<div class="absolute left-0 mt-[20px] mx-auto flex h-20 w-full items-center justify-center border-gray-300 md:relative md:mx-0 md:w-auto">
			...
			<div class="flex w-full items-center justify-between gap-4 md:w-auto">
				...
				>
					<div class="absolute top-0 flex w-full justify-center bg-secondary-foreground py-2.5 uppercase text-yellow-400 md:hidden">
					...
					{#each currentUser.name ? navItemsLogado : navItems as item}
						{#if $page.url.pathname === item.href}
							<a
								href={item.href}
								class="lg:max-sm:w-[96px] w-full bg-yellow-400 py-2 text-center font-['Inter']
						...
						{:else}
							<a
								href={item.href}
								class="lg:max-sm:w-[96px] w-full bg-purple-200 py-2 text-center font-['Inter']
								text-[12px] font-semibold text-purple-950 transition-colors
								hover:no-underline hover:opacity-80 sm:min-w-[90px] sm:rounded-sm md:min-w-20 md:p-1 lg:w-auto"
							>
								<!-- Horizontal layout: icon then name -->
					...
					{#if !!currentUser.photoURL}
						<a href="/perfil">
							<div class="flex min-w-8 items-center">
								<!-- svelte-ignore component_name_lowercase -->
								<img src={currentUser.photoURL || "/placeholder.svg"} alt="Foto de perfil" class="h-8 w-8 rounded-full" />
							</div>
						</a>
					{:else}
						<div class="flex items-center rounded-md bg-secondary-foreground bg-opacity-60 p-1.5">
							<a href="/login" class="text-secondary flex items-center gap-1">
