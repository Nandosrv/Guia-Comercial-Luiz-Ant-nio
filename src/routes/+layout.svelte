...
<div
	class="relative m-auto h-full min-h-screen w-full max-w-6xl justify-between bg-purple-950/100 px-4"
>
	<nav class="flex h-20 w-full items-center justify-between px-2">
		<!-- Logo -->
		<div
		...
		<div class="flex w-full items-center justify-between gap-4 md:w-auto">
			...
			{[
					isOpen
						? 'dark:bg-background z-20 h-full translate-x-0 flex-col gap-4 bg-black bg-opacity-80 dark:bg-opacity-70'
			...
			>
				...
				{#each currentUser.name ? navItemsLogado : navItems as item}
					{#if $page.url.pathname === item.href}
						<a
							href={item.href}
							class="w-full bg-yellow-400 py-4 text-center font-['Inter'] text-[12px]
		font-semibold text-purple-950 transition-colors hover:no-underline
		 hover:opacity-80 sm:min-w-[90px] sm:rounded-sm md:min-w-24 md:p-1 lg:w-[90px] lg:max-sm:w-[96px]"
						>
							<span>{item.name}</span>
						</a>
					{:else}
						<a
							href={item.href}
							class="w-full bg-purple-200 py-4 text-center font-['Inter'] text-[12px]
		font-semibold text-purple-950 transition-colors hover:no-underline
		 hover:opacity-80 sm:min-w-[90px] sm:rounded-sm md:min-w-24 md:p-1 lg:w-[90px] lg:max-sm:w-[96px]"
						>
							<span>{item.name}</span>
