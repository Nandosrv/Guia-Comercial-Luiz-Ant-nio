...
<main class="mt-4 flex w-full flex-col items-center">
    ...
    <div class="w-[90%] max-w-[600px] rounded-lg bg-white p-4 shadow-md">
        ...
        <div class="mt-3 flex justify-between">
            <button class="flex items-center space-x-2 rounded-md px-4 py-2 text-blue-600 hover:bg-blue-50" on:click={openGallery}>
        ...
        {#each posts as post, index (post.id)}
            <div class="rounded-lg bg-white p-4 shadow-md">
                ...
                {#if post.image_url}
                    ...
                    <img src={post.image_url} alt="Imagem do post" class="mt-3 w-full rounded-md object-cover" on:click={() => openImageModal(post.image_url)} />
                {/if}
                <div class="mt-4 flex items-center space-x-6">
                    <button on:click={() => likePost(index)} class="flex items-center space-x-2 text-red-600">
                        <span class="text-lg">❤️</span>
                        <span>{post.likes}</span>
                    </button>
                    <button class="flex items-center space-x-2 text-gray-600" on:click={() => toggleComments(index)}>
                    ...
                    <div class="mt-3 space-y-3">
                        {#each post.comments as comment}
                            <div class="flex items-start space-x-3">
                                <img src={comment.avatar} alt={comment.user} class="h-8 w-8 rounded-full object-cover" />
                                <div>
                ...
                {#if post.showComments}
                    <div class="mt-3">
                        <textarea bind:value={commentText} placeholder="Adicione um comentário..." class="w-full rounded-md border border-gray-300 p-2"></textarea>
                        <button on:click={() => addComment(index)} class="mt-2 rounded-md bg-blue-600 px-4 py-2 text-white hover:bg-blue-700">
                            Enviar
                        </button>
                    </div>
                {/if}
    ...
    {#if showImageModal}
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div class="relative w-[90%] max-w-[500px]">
                <button class="absolute top-0 right-0 p-2 text-white bg-gray-700 rounded-full" on:click={closeImageModal}>X</button>
