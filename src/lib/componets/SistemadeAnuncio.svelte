...
<main
	class="container mx-auto rounded-lg bg-white px-4 py-8 transition-colors duration-300 dark:bg-secondary {darkMode
		? 'dark:bg-gray-900 dark:text-white'
		: ''}"
>
	<div class="mb-8 flex items-center justify-between border-b-2 dark:border-secondary-foreground">
		<h1 class="font-['Inter'] text-4xl font-bold text-black dark:text-white lg:text-5xl">
	...
	<section class="grid grid-cols-1 gap-6 md:grid-cols-3">
		{#each filteredAds as ad (ad.id)}
			...
			>
				...
				<div class="p-6">
					<h3 class="mb-2 text-xl font-semibold">{ad.business_name}</h3>
					<p class="mb-2 text-gray-600 dark:text-gray-300">{ad.description}</p>
					<div class="flex items-center">
						<!-- Rating precisa ser ajustado dinamicamente -->
						<span class="mr-1 text-yellow-400">⭐⭐⭐⭐</span>
	...
	{#if showModal}
		...
		>
			<div
				class="h-[70%] max-h-screen w-full max-w-lg overflow-y-auto rounded-lg border bg-white p-6 shadow-lg dark:border-secondary-foreground dark:bg-gray-800 md:max-w-xl"
			>
				...
				<form on:submit|preventDefault={submitAd}>
					...
					<label for="phoneNumber" class="mb-2 block text-sm dark:text-secondary"
